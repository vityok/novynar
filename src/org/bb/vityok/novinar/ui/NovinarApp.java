...
public class NovinarApp extends Application {
	...
	private void buildChildren() {
		...
		if (outlines != null) {
		    ...
		    for (Outline childOutline : outlines) {
	...
	Outline root = OPMLManager.getInstance().getRootOutline();
	TreeItem<Outline> rootItem = new OutlineTreeItem(root);
        rootItem.setExpanded(true);
        channelsTree = new TreeView<Outline> (rootItem);
     ...
     * the channels tree.
     */
    private void updateItemsTable(Channel chan) {
        try {
            NewsItemDAO dao = NewsItemDAO.getInstance();
            ObservableList<NewsItem> items = FXCollections.observableArrayList(dao.getNewsItemByChannel(chan));
            itemsTable.setItems(items);
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     */
    private void selectedOutline(Outline outline) {
